...
JSON的值可以是字符串、数值、布尔值、null、对象、数组。

JSON可以独立成文件，扩展名为json。

JSON的键与值如果是字符串，那么则一定是双引号

## JavaScript中的JSON

1. json字符串

   数据格式符合JSON格式的要求，类型是字符串
...
1. 将json格式字符串转为JSON对象
   ...
   ![image-20200822142854868](https://files.alexhchu.com/2020/08/22/64bc082feff64.png)
...
2. 将json对象转换为JSON字符串
   ...
   ![image-20200902171109711](https://files.alexhchu.com/2020/09/02/8598695092fb3.png)

### 兼容问题

如果浏览器没有此对象，那么可以使用[JSON2文件](https://github.com/douglascrockford/JSON-js)引入JSON对象，在进行此操作。
...
当发送数据时，最简单的方式就是进行拼串，而不是将JSON对象转换为对象。
...
<html lang="en">
    ...
    <body>
        ...
        <script src="1.%20createXMLHttpRequest.js"></script>
        <script>
            var btn = document.getElementById('btn')
            btn.addEventListener('click', function () {
                var xhr = createXMLHttpRequest()
                xhr.open('get', '3. xml数据格式.xml')
                xhr.send(null)
                xhr.onreadystatechange = function () {
                    if (xhr.readyState === 4 && xhr.status === 200) {
                        //    接收数据
...
![image-20200822150149576](https://files.alexhchu.com/2020/08/22/e23e7b009ea86.png)
...
2. 将获取到的数据进行处理
   - 首先遍历全部数组，将省份填入到选项框
   - 当用户选择后（触发change事件）根据用户选择的省份将城市信息填入其中。
